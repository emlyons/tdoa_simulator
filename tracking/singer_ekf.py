...
class SINGER_EKF(EKFBase):
    def __init__(self, state, P, Q, R, tau, S, c):
    ...
    def measurement_model_jacobian(self):
        ...
        r_1 = np.linalg.norm(np.array([x, y]) - self.S[1])
        r_2 = np.linalg.norm(np.array([x, y]) - self.S[2])
        r_3 = np.linalg.norm(np.array([x, y]) - self.S[3])
        r_ref = np.linalg.norm(np.array([x, y]) - self.S[0])

        H = np.array([
            [(x - self.S[1][0]) / r_1 - (x - self.S[0][0]) / r_ref, 0.0, 0.0,
             (y - self.S[1][1]) / r_1 - (y - self.S[0][1]) / r_ref, 0.0, 0.0],
            [(x - self.S[2][0]) / r_2 - (x - self.S[0][0]) / r_ref, 0.0, 0.0,
             (y - self.S[2][1]) / r_2 - (y - self.S[0][1]) / r_ref, 0.0, 0.0],
            [(x - self.S[3][0]) / r_3 - (x - self.S[0][0]) / r_ref, 0.0, 0.0,
             (y - self.S[3][1]) / r_3 - (y - self.S[0][1]) / r_ref, 0.0, 0.0],
        ], dtype=float)
        H = H / self.c
        return H
    ...
    def measurement_model(self):
        ...
        d_ref = np.linalg.norm(pos - np.asarray(self.S[0], dtype=float))
        dt_s1 = (1.0 / self.c) * (np.linalg.norm(pos - np.asarray(self.S[1], dtype=float)) - d_ref)
        dt_s2 = (1.0 / self.c) * (np.linalg.norm(pos - np.asarray(self.S[2], dtype=float)) - d_ref)
        dt_s3 = (1.0 / self.c) * (np.linalg.norm(pos - np.asarray(self.S[3], dtype=float)) - d_ref)
